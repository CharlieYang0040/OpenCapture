...
3. WASAPI 시스템 소리와 마이크, 오디오 인코딩 및 A/V 동기화를 구현한다.
4. 임시 MKV, 저장 공간 검사, 일시정지, 복구 및 MP4 remux를 구현한다.
...
- 48kHz 공통 변환, 시스템·마이크 믹서, AAC/Opus 인코딩, muxer 연결, 장치 선택 및 장시간 드리프트 보정은 다음 작업이다.
- 현재 UI의 시스템 소리와 마이크 체크박스는 최종 영상 파일의 오디오 트랙과 아직 연결되지 않았다.
