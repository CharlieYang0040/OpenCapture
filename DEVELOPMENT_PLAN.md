...
2026-07-24 점검 기준으로 GPU H.264/AAC MKV 녹화, 스크린샷, 클립보드 캡처, 안전 임시 저장, 미완료 MKV 복구, 일시정지, MP4 remux 및 영역 프리셋 기반 GIF가 실제 UI에 연결되었다. 범용 오프라인 변환과 장시간 성능 검증은 아직 남아 있다.
...
5. 영역 프리셋 기반 GIF를 구현한다. (핵심 기능, 용량 제어 및 스모크 완료)
5-1. GIF 비동기 진행률·취소를 구현한다. (완료)
5-2. VP9/Opus를 추가하고 MP4/MKV/WebM 범용 오프라인 변환을 구현한다.
6. 저부하 모드, 성능 측정, QSV/AMF, Desktop Duplication과 트레이를 검증·완성한다.
   (캡처·비디오·GIF 전역 단축키의 설정·저장·충돌 방지는 구현 완료)
