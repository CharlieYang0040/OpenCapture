...
단축키도 동작한다. 다만 영역 선택 가독성, 여러 모니터를 가로지르는 영역의 정확성,
표시만 되고 동작하지 않는 옵션, 저부하 실측 및 배포 준비가 남아 있다.
...
0. 영역 선택 가독성과 정확성을 보완한다. 선택 안쪽을 밝게 유지하고 바깥 어둡기 설정,
   고대비 선, 모니터별 안내, 단일 모니터 경계 및 혼합 DPI를 검증한다.
...
|---|---|---|---|
| P0 | 영역 선택 가독성 | 전체 화면을 검정색으로 칠한 뒤 alpha 150/255를 적용해 선택 안쪽까지 약 59% 어둡다. | 선택 바깥만 기본 30% 어둡게 하고 내부는 투명하게 유지한다. 어둡기 조절, 고대비 선 및 실기 가독성 시험을 추가한다. |
| P0 | 다중 모니터 영역 | WGC는 선택 영역과 연결된 한 모니터만 캡처하고 crop은 그 프레임에 맞춰 제한하므로 경계를 넘긴 영역은 잘릴 수 있다. | 단기에는 한 모니터 안으로 선택을 제한한다. 장기에는 모니터별 WGC/DXGI 프레임을 GPU에서 합성한 뒤 교차 모니터 선택을 허용한다. |
| P0 | UI 신뢰성 | 커서, HEVC 및 저부하 컨트롤이 동작하는 것처럼 보이지만 엔진과 연결되지 않았다. | 실제 연결 전에는 비활성화하고 이유를 툴팁에 표시한다. 활성 컨트롤은 자동 또는 수동 결과 검증을 가져야 한다. |
| P1 | 선택 상태 충돌 | 영역 선택 중 이전 상시 테두리와 전역 단축키가 새 선택 흐름과 혼동 또는 충돌할 수 있다. | picker가 열리면 기존 테두리와 일반 단축키를 일시 중단하고 확정·취소·오류 모든 경로에서 복원한다. |
