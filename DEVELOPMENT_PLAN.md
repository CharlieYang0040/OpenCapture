...
1. 커서, HEVC 및 저부하처럼 표시만 되는 옵션은 실제로 연결하거나 연결 전까지 비활성화하고
   이유를 툴팁에 표시한다. 동작하지 않는 컨트롤을 클릭 가능한 상태로 두지 않는다.
2. 공식 Release 출력은 `build/repo-ninja-x64-release`로 통일한다. `release-verification`
   같은 임시 검증 폴더는 문서·VS Code·스크립트의 기본 경로로 사용하지 않고, 잠긴 실행
   파일을 검증할 때만 폐기 가능한 별도 staging 디렉터리를 사용한다.
3. 저부하 모드와 성능 계측을 연결하고 NVIDIA, Intel, AMD 및 혼합 GPU에서 1080p/1440p/4K,
   게임 1% low, 장시간 A/V 동기화 합격 기준을 실측한다.
4. WGC 실패와 borderless 권한 제한을 처리할 Desktop Duplication 폴백을 구현하고,
   영역·모니터 테두리가 결과에 들어가지 않는 패키징 시나리오를 검증한다.
5. 오디오 장치 선택, 볼륨·음소거, Opus, 장치 변경 복구 및 2시간 드리프트 보정을 완성한다.
6. VP9/Opus와 MP4/MKV/WebM 범용 오프라인 변환, QSV/AMF를 완성한다.
7. 남은 단축키 동작, 트레이, 설정 통합·마이그레이션, 세션 로그와 진단 내보내기를 추가한다.
8. 자동 UI 회귀 시험, 배포 패키지, 의존성 고지, 서명 및 회사 PC 보안 안내를 완성한다.
...
- 트레이, 즐겨찾기 프리셋별 단축키, 마이크 음소거 단축키와 직접 키 입력 방식은 후속 작업이다.

### 12.13 13단계: Desktop Duplication 폴백
