...
- 캡처 대상을 탭 밖의 공통 소스 바로 통합하고 대상 선택·영역 프리셋을 단일 모달로 정리
- 캡처/비디오/GIF의 시작과 녹화 중 일시정지·중지를 같은 하단 세션 명령 바로 통합
- 팝업과 탭에 번역과 무관한 고정 ID를 적용해 대상 모달 및 한영 전환 상태를 안정화
- 마우스 클릭 포커스가 툴팁을 붙잡지 않게 하고 키보드 탐색 포커스만 유지
